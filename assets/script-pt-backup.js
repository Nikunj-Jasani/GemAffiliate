...
// Remove duplicate function - using the unified version below
...
// Dashboard utility functions - Unified showMessage for all contexts
function showMessage(message, type) {
    ...
    alertDiv.className = `affiliate-message-alert affiliate-alert-${type}`;
    alertDiv.setAttribute('role', 'alert');
    alertDiv.innerHTML = `
        <div class="affiliate-alert-content">
            <div class="affiliate-alert-icon">
                ${type === 'success' ? 
                    '<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M9,20.42L2.79,14.21L5.62,11.38L9,14.77L18.88,4.88L21.71,7.71L9,20.42Z"/></svg>' :
            ...
            <div class="affiliate-alert-message">${message}</div>
            <button type="button" class="affiliate-alert-close" onclick="this.parentElement.parentElement.remove()">&times;</button>
        </div>
    `;
    
    // Find the right container - form first, then dashboard, then body
    let container = document.querySelector('.affiliate-form-container');
    if (!container) container = document.querySelector('.affiliate-dashboard-content');
    if (!container) container = document.body;
    
    container.insertBefore(alertDiv, container.firstChild);
    
    // Apply styles if not already added
    if (!document.getElementById('affiliate-alert-styles')) {
        const styleEl = document.createElement('style');
        styleEl.id = 'affiliate-alert-styles';
        styleEl.textContent = `
            ...
            .affiliate-alert-content {
                display: flex;
                align-items: center;
                padding: 15px 20px;
            }
            .affiliate-alert-icon {
                flex-shrink: 0;
                margin-right: 12px;
                opacity: 0.8;
            }
    ...
    if (confirm('Are you sure you want to logout?')) {
        ...
        .then(data => {
            if (data.success) {
                showMessage('Logged out successfully!', 'success');
                setTimeout(() => {
                    window.location.href = data.data.redirect || '/affiliate-login';
                }, 1000);
            } else {
                showMessage(data.data || 'Logout failed', 'danger');
            }
        })
        .catch(error => {
            showMessage('An error occurred. Please try again.', 'danger');
