...
if (!$current_user) {
    error_log("Portuguese Individual KYC Form: User not found in database. User ID: " . $user_id);
...
<div class="kyc-form-container">
    ...
    <div class="kyc-form-header">
        ...
        <h2>Verificação KYC Individual</h2>
    </div>
    ...
    <div class="kyc-account-type-display">
        <div class="account-type-badge individual">
            ...
            <span>Tipo de Conta: <strong>Individual</strong></span>
        </div>
    </div>
    ...
    <div class="kyc-progress-indicator">
        <div class="kyc-progress-text">Aplicação KYC Individual</div>
    ...
    <form id="kycIndividualForm" enctype="multipart/form-data">
        ...
        <div class="kyc-form-grid">
            ...
            <div class="kyc-form-group">
                <label class="kyc-form-label required" for="kyc_email">Endereço de E-mail</label>
            ...
            <div class="kyc-form-group">
                <label class="kyc-form-label required" for="kyc_mobile_number">Número de Celular</label>
            ...
            <div class="kyc-form-group">
                ...
                <select id="kyc_affiliate_type" name="affiliate_type" class="kyc-form-input" required>
                    ...
                    <option value="Influencer" <?php selected($form_data->affiliate_type ?? '', 'Influencer'); ?>>Influenciador</option>
                    <option value="Content Creator" <?php selected($form_data->affiliate_type ?? '', 'Content Creator'); ?>>Criador de Conteúdo</option>
                    <option value="Blogger" <?php selected($form_data->affiliate_type ?? '', 'Blogger'); ?>>Blogueiro</option>
                    <option value="Website Owner" <?php selected($form_data->affiliate_type ?? '', 'Website Owner'); ?>>Proprietário de Website</option>
                    <option value="Social Media Marketer" <?php selected($form_data->affiliate_type ?? '', 'Social Media Marketer'); ?>>Marketing de Redes Sociais</option>
                    <option value="Email Marketer" <?php selected($form_data->affiliate_type ?? '', 'Email Marketer'); ?>>Marketing por E-mail</option>
                    <option value="Other" <?php selected($form_data->affiliate_type ?? '', 'Other'); ?>>Outro</option>
                </select>
            </div>
        </div>
        ...
        <div class="kyc-form-group" style="grid-column: 1 / -1;">
            <label class="kyc-form-label" for="kyc_affiliate_urls">
                ...
                URLs de Sites para Aplicação de Afiliado
            </label>
            <textarea id="kyc_affiliate_urls" name="affiliate_urls" class="kyc-form-input" 
                      ...
                      placeholder="Digite os URLs dos seus sites (um por linha):&#10;https://exemplo.com&#10;https://blog.exemplo.com&#10;https://perfil-redes-sociais.com"><?php echo esc_textarea($form_data->affiliate_urls ?? ''); ?></textarea>
            <small style="color: #6c757d; font-size: 0.9rem; margin-top: 8px; display: block;">
                📝 Liste todos os sites, blogs, perfis de redes sociais ou plataformas onde pretende promover conteúdo de afiliado. Isso nos ajuda a entender seus canais de promoção.
            </small>
        </div>
        ...
        <div class="kyc-form-grid">
            ...
            <div class="kyc-form-group">
                ...
                <input type="text" id="kyc_address_line2" name="address_line2" class="kyc-form-input" 
                       ...
                       placeholder="Apartamento, suíte, andar, etc.">
            </div>
        ...
        <div class="kyc-documents-section">
            ...
            <div class="kyc-form-grid">
                <!-- Identity Document -->
                <div class="kyc-form-group">
                    <label class="kyc-form-label required" for="kyc_identity_document_type">Tipo de Documento de Identidade</label>
                    <select id="kyc_identity_document_type" name="identity_document_type" class="kyc-form-input" required>
                        <option value="">Selecione o Tipo de Documento</option>
                        <option value="Passport" <?php selected($form_data->identity_document_type ?? '', 'Passport'); ?>>Passaporte</option>
                        <option value="National ID" <?php selected($form_data->identity_document_type ?? '', 'National ID'); ?>>RG / Carteira de Identidade</option>
                        <option value="Driver's License" <?php selected($form_data->identity_document_type ?? '', 'Driver\'s License'); ?>>Carteira de Motorista</option>
                        <option value="CPF" <?php selected($form_data->identity_document_type ?? '', 'CPF'); ?>>CPF</option>
                    </select>
                </div>

                <div class="kyc-form-group">
                    <label class="kyc-form-label required" for="kyc_identity_document_number">Número do Documento</label>
                ...
                <div class="kyc-form-group">
                    <label class="kyc-form-label" for="kyc_identity_document_expiry">Data de Validade do Documento</label>
                ...
                <!-- Document Uploads -->
                <div class="kyc-form-group">
                    <label class="kyc-form-label required" for="kyc_identity_document">Upload do Documento de Identidade</label>
                    <input type="file" id="kyc_identity_document" name="identity_document" class="kyc-file-input" 
                           accept=".jpg,.jpeg,.png,.pdf" required>
                    <small style="color: #6c757d;">Envie uma foto clara ou digitalização do seu documento de identidade (JPG, PNG, PDF)</small>
                </div>

                <div class="kyc-form-group">
                    <label class="kyc-form-label required" for="kyc_address_proof">Comprovante de Endereço</label>
                    <select id="kyc_address_proof_type" name="address_proof_type" class="kyc-form-input" style="margin-bottom: 10px;" required>
                        ...
                        <option value="Utility Bill" <?php selected($form_data->address_proof_type ?? '', 'Utility Bill'); ?>>Conta de Luz/Água/Gás</option>
                        <option value="Bank Statement" <?php selected($form_data->address_proof_type ?? '', 'Bank Statement'); ?>>Extrato Bancário</option>
                        <option value="Government Letter" <?php selected($form_data->address_proof_type ?? '', 'Government Letter'); ?>>Documento Governamental</option>
                        <option value="Lease Agreement" <?php selected($form_data->address_proof_type ?? '', 'Lease Agreement'); ?>>Contrato de Aluguel</option>
                    </select>
                    <input type="file" id="kyc_address_proof" name="address_proof" class="kyc-file-input" 
                           accept=".jpg,.jpeg,.png,.pdf" required>
                    <small style="color: #6c757d;">Envie comprovante de endereço recente com até 3 meses (JPG, PNG, PDF)</small>
                </div>

                <div class="kyc-form-group">
                    <label class="kyc-form-label required" for="kyc_selfie">Verificação por Selfie</label>
                    <input type="file" id="kyc_selfie" name="selfie" class="kyc-file-input" 
                           accept=".jpg,.jpeg,.png" required>
                    <small style="color: #6c757d;">Envie uma selfie clara segurando seu documento de identidade (JPG, PNG)</small>
                </div>
            </div>
        </div>
        ...
        <div class="kyc-submit-section">
            ...
            <p style="font-size: 0.9rem; color: #6c757d; margin: 0;">
                Ao enviar este formulário, você confirma que todas as informações fornecidas são precisas e concorda com nosso processo de verificação KYC.
            </p>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        .then(data => {
            if (data.success) {
                alert('Aplicação KYC enviada com sucesso!');
                ...
                alert('Falha ao enviar KYC: ' + data.data);
            }
        })
        .catch(error => {
            console.error('Erro ao enviar KYC:', error);
            alert('Erro ao enviar aplicação KYC');
        })
        .finally(() => {
